...
// --- DONNÉES DE L'ARBRE DE COMPÉTENCES ---
const skillsData = [
  {
    id: "1",
...
// --- NOUVELLE DEFINITION DES NIVEAUX (AVEC HEURES) ---
const LEVEL_DETAILS = [
...
// --- DEFINITION DES PACKAGES (PRESETS) ---
const PRESETS = [
  // --- NIVEAU 1 : INITIATION ---
  {
  ...
  // --- NIVEAU 2 : MODULES EXPERTS (Courts 2-4j) ---
  {
    id: 'focus_qualif',
    label: 'Qualité',
  ...
  // --- NIVEAU 3 : PARCOURS MÉTIERS (Longs 10j+) ---
  {
...
const SkillItemSelectable = ({ subSkill, isSelected, onToggle }) => (
  ...
  >
    ...
    <div>
      <h3 className={`text-sm font-semibold mb-1 ${isSelected ? 'text-blue-900' : 'text-slate-800'}`}>
      ...
      {LEVEL_DETAILS.map((level, index) => {
        ...
        return (
          <button
            ...
            `}
          >
            <div className={`text-xs font-bold uppercase tracking-wider mb-1 ${isSelected ? 'text-blue-100' : 'text-slate-400'}`}>
              Niveau {index}
            </div>
            ...
            <div className="bg-white p-4 rounded-lg border border-slate-200 shadow-sm mb-6 sticky top-4 z-20 flex flex-col gap-4">
              ...
              <div className="flex flex-col md:flex-row gap-3 items-center w-full">
                <div className="relative flex-grow w-full">
                  ...
                  <input
                    type="text"
                    className="w-full pl-10 pr-4 py-2 border border-slate-200 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 bg-slate-50"
      ...
      case 1: // PRIORISATION
        if (selectedSkills.length === 0) return (
            <div className="text-center py-20 bg-white rounded-lg border border-slate-200 border-dashed">
                ...
                <button onClick={() => setActiveTab(0)} className="text-blue-600 font-medium mt-2 hover:underline">Retourner à la sélection</button>
            </div>
        );
        return (
          <div className="animate-in slide-in-from-right-4 duration-300">
            ...
            <div className="space-y-2">
              {resolvedSelectedSkills.map((skill, index) => (
                <div key={skill.id} className="bg-white p-3 border border-slate-200 rounded-lg shadow-sm flex items-center justify-between group hover:border-blue-400 transition-colors">
        ...
        return (
          <div className="animate-in slide-in-from-right-4 duration-300 max-w-2xl mx-auto">
            <div className="bg-white p-8 border border-slate-200 rounded-xl shadow-lg">
              <h2 className="text-2xl font-bold text-slate-900 mb-6">Planification de la Formation</h2>
                     ...
                     {PACE_OPTIONS.map((option) => {
                       ...
                       return (
                         ...
                         >
                            ...
                            <span className={`text-[11px] font-medium mt-1 ${isSelected ? 'text-blue-100' : 'text-slate-500'}`}>{option.sub}</span>
                         </button>
                       )
                     })}
                   </div>
                   ...
                   <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                     {FORMAT_OPTIONS.map((option) => {
                       ...
                       return (
                         ...
                         >
                            <span className="text-sm font-bold leading-tight">{option.label}</span>
                            <span className={`text-[11px] font-medium mt-1 ${isSelected ? 'text-indigo-100' : 'text-slate-500'}`}>{option.sub}</span>
                         </button>
                       )
                     })}
                   </div>
            ...
            <div className="mt-6 flex justify-center">
              <button 
                className="bg-green-600 text-white px-8 py-3 rounded-lg font-bold shadow-lg hover:bg-green-700 transition-colors flex items-center gap-2"
                onClick={() => alert("Dossier de formation généré (simulation)")}
              >
        ...
        <div className="max-w-5xl mx-auto">
          <div className="flex flex-col md:flex-row items-center justify-between px-4 py-4 md:py-0 md:h-20 gap-4">
             <div className="flex items-center gap-3 shrink-0 self-start md:self-auto pt-2 md:pt-0">
                <div className="p-2 bg-blue-700 rounded-lg shadow-sm text-white">
                   <MessageCircle size={24} />
                </div>
                <div>
                   <p className="text-xs font-semibold text-blue-600 uppercase tracking-wide mb-0.5">Entretien de découverte</p>
                  ...
                  return (
                    ...
                    >
                      ...
                      {idx === 0 && selectedSkills.length > 0 && (
                        <span className={`ml-1 text-[10px] px-1.5 rounded-full ${isActive ? 'bg-white text-blue-600' : 'bg-blue-200 text-blue-800'}`}>
